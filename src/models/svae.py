...
class SpatialVAE(pl.LightningModule):
  ...
  def __init__(self,
    ...
    Args:
    n_uncostrained: int
      The number of uncostrained latent variables. The encoder will output a
      mean and a logged standard deviation for each uncostrained latent
      variable.
    """
    super().__init__()
  ...
  def encode(self, x):
    ...
    logstd: torch.tensor
      ...
      deviations of uncostrained latent variables, theta (optional), and delta_x
      (optional).
    """
  ...
  def sample(self, mu, logstd):
    ...
    Returns:
    z: torch.tensor
      A tensor of shape [batch_size, self.n_outputs_encoder / 2]. Uncostrained
  ...
  def decode(self, z):
    ...
    Extracts theta and delta_x from z. Generates normalized grid coordinates and
    transforms them (rotation + translation). Concatenates the transformed
    ...
    # Transformation matrices to be applied to the normalized grid coordinates
  ...
  def forward(self, x):
    ...
    # Want to flatten the input to [batch_size, n_channels * width * height] for
    # the encoder.
